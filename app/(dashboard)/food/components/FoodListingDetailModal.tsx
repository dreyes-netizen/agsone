    ...
    >
      ...
      >
        {listing && (() => {
          ...
          const orderTotal = computeOrderTotal(parseFloat(listing.price), { quantity: qty, selectedAddOns });
            ...
            <>
              ...
              <div className="overflow-y-auto scrollbar-hide flex-1 rounded-2xl">
                ...
                <div className="relative group">
                  <button
                    type="button"
                    onClick={() => onOpenLightbox(images, imageIndex)}
                    disabled={total === 0}
                    className="block w-full aspect-[4/3] cursor-zoom-in focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-emerald-500 rounded-t-2xl disabled:cursor-default"
                    aria-label={total > 0 ? `Zoom in on ${listing.title} image ${imageIndex + 1} of ${total}` : listing.title}
                  >
                    <FoodImage
                  ...
                  {total > 1 && (
                    <>
                      <button
                        ...
                        onClick={(e) => { e.stopPropagation(); onImageIndexChange((imageIndex - 1 + total) % total); }}
                      ...
                      <button
                        ...
                        onClick={(e) => { e.stopPropagation(); onImageIndexChange((imageIndex + 1) % total); }}
                      ...
                      <div className="absolute bottom-2.5 left-1/2 -translate-x-1/2 flex gap-2">
                        {images.map((_, i) => (
                          <button
                            ...
                            onClick={(e) => { e.stopPropagation(); onImageIndexChange(i); }}
                ...
                <div className="p-5 space-y-4">
                  ...
                  {/* Seller avatar + name + department + order count */}
                  <div className="flex items-center justify-between gap-3">
                    ...
                    >
                      ...
                      <span className="text-xs text-gray-500 truncate">
                        by {listing.createdBy.displayName}
                      ...
                      {listing.addOns.map((a, i) => (
                        <div key={i} className="flex justify-between text-xs text-gray-700">
                          ...
                          <span className="font-semibold text-amber-700">
                            {a.price > 0 ? `+₱${a.price % 1 === 0 ? a.price : a.price.toFixed(2)}` : "Free"}
                          </span>
                        </div>
                      ))}
                    </div>
                  ...
                  {listing.deliveryDate && (
                    <p className="flex items-center gap-1 text-xs text-sky-600 font-medium">
                      <Truck className="w-3.5 h-3.5 shrink-0" aria-hidden="true" /> Delivery: {formatDateTime(listing.deliveryDate)}
                    </p>
                  )}
                  ...
                  <div className="pt-3 border-t border-gray-100 space-y-3">
                    ...
                    {!closed && !isMine && !hasOrder && orderMode === "confirm" && (() => {
                      ...
                      const baseSubtotal = basePrice * qty;
                      return (
                        <div className="space-y-4">
                          ...
                          <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm">
                            <div className="flex justify-between gap-3">
                              <span className="text-gray-500 shrink-0">Item</span>
                              <span className="font-semibold text-gray-900 text-right max-w-[60%] leading-snug">{listing.title}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-500">{formatPrice(listing.price)} × {qty}</span>
                              <span className="font-semibold text-gray-900 tabular-nums">₱{baseSubtotal.toFixed(2)}</span>
                            </div>
                            {selectedAddOns.map((a, i) => (
                              <div key={i} className="flex justify-between">
                                <span className="text-gray-500">
                                  + {a.name} {a.price > 0 ? `₱${a.price % 1 === 0 ? a.price : a.price.toFixed(2)} × ${qty}` : "(free)"}
                                </span>
                                <span className="font-semibold text-amber-700 tabular-nums">
                                  {a.price > 0 ? `₱${(a.price * qty).toFixed(2)}` : "Free"}
                                </span>
                              </div>
                            ))}
                            {orderNote && (
                            ...
                            <div className="border-t border-gray-200 pt-2 flex justify-between">
                              ...
                              <span className="font-bold text-emerald-700 text-base tabular-nums">₱{orderTotal.toFixed(2)}</span>
                            </div>
                          </div>
                    ...
                    {!isMine && hasOrder && orderMode !== "edit" && (() => {
                      ...
                      const oTotal = computeOrderTotal(parseFloat(listing.price), myOrder);
                      return (
                        <div className="rounded-lg bg-emerald-50 border border-emerald-100 p-3 space-y-2">
                          <div className="flex items-start justify-between gap-3">
                            <div className="space-y-1 flex-1 min-w-0">
                              ...
                              {(myOrder.selectedAddOns?.length ?? 0) > 0 && (
                                <div className="space-y-0.5">
                                  {myOrder.selectedAddOns.map((a, i) => (
                                    <p key={i} className="text-xs text-amber-700">
                                      + {a.name} <span className="text-gray-500">(₱{a.price.toFixed(2)} × {myOrder.quantity})</span>
                                    </p>
                                  ))}
                                </div>
                              )}
                          ...
                          {myOrder.note && (
                            <p className="text-xs text-gray-500 italic border-t border-emerald-100 pt-2">
                              <span aria-hidden="true">📝 </span>&ldquo;{myOrder.note}&rdquo;
                            </p>
                          )}
                          {!closed && (
