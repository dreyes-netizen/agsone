...
interface MyRequestsViewProps {
  ...
  onOpenDetail: (redemption: Redemption) => void;
}
...
export function MyRequestsView({ loading, redemptions, onOpenDetail }: MyRequestsViewProps) {
  if (loading) {
    return (
  ...
  if (redemptions.length === 0) {
    return (
      <MarketplaceEmptyState
        ...
        description="Rewards you request will appear here."
      />
    );
  }
  ...
  return (
    <>
      ...
      <div className="hidden md:block bg-white rounded-card border border-table-border overflow-clip">
        <table className="w-full border-collapse" aria-label="My reward requests">
          <thead className="bg-table-head">
            <tr className="border-b border-table-border">
              <th scope="col" className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500 first:pl-5">Reward</th>
              <th scope="col" className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500">Type</th>
              <th scope="col" className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500">Points</th>
          ...
          <tbody>
            {redemptions.map((r, i) => (
              <tr key={r.id} className={`border-b border-row-border last:border-b-0 transition-colors hover:bg-row-hover ${i % 2 === 1 ? "bg-row-alt" : ""}`}>
                <td className="px-4 py-3 text-sm font-medium text-gray-900 first:pl-5">{r.reward.name}</td>
                <td className="px-4 py-3">
                  <RewardTypeBadge category={r.reward.category} />
                </td>
                <td className="px-4 py-3 text-sm font-semibold text-navy-600 tabular-nums">{r.pointsSpent.toLocaleString()} pts</td>
          ...
          <li key={r.id}>
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <p className="font-semibold text-gray-900 leading-snug">{r.reward.name}</p>
                <RequestStatusBadge status={r.status} />
              </div>
              <div className="flex items-center gap-2 text-xs text-gray-500">
                <RewardTypeBadge category={r.reward.category} />
                <span aria-hidden="true">·</span>
                <span className="font-medium tabular-nums">{r.pointsSpent.toLocaleString()} pts</span>
              </div>
