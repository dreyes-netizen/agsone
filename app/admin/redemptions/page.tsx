          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="text-center py-8 text-gray-400">No pending redemptions.</td>
                </tr>
              ) : pending.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50/60 transition-colors border-b border-gray-50">
                  ...
                  <td className={tdClass}>
                    ...
                    <p className="text-xs text-gray-400">{r.reward.category}</p>
                  </td>
                  <td className={tdClass}>
                  ...
                  <td className={tdClass}>
                    <div className="flex gap-1.5">
                      <button
                        className="bg-emerald-600 text-white px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-emerald-700 disabled:opacity-50 flex items-center gap-1"
                      ...
                      <button
                        className="bg-red-50 text-red-600 border border-red-200 px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-red-100 disabled:opacity-50 flex items-center gap-1"
                        disabled={actionId === r.id}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {processed.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-8 text-gray-400">No processed redemptions yet.</td>
                </tr>
              ) : processed.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50/60 transition-colors border-b border-gray-50">
                  <td className={tdClass}>
                  ...
                  <td className={`${tdClass} text-xs text-gray-400 max-w-xs truncate`}>{r.adminNote ?? "—"}</td>
                  <td className={tdClass}>
                    {r.status === "APPROVED" && (
                      <button
                        className="bg-blue-50 text-blue-600 border border-blue-200 px-3 py-1.5 rounded-lg text-xs font-semibold disabled:opacity-50 flex items-center gap-1"
                        disabled={actionId === r.id}
