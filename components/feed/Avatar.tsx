...
export function Avatar({ name, url, size = "sm" }: { name: string; url: string | null; size?: "xs" | "sm" | "md" }) {
  const [errored, setErrored] = useState(false);
  const dim = size === "md" ? "w-12 h-12 text-lg" : size === "xs" ? "w-8 h-8 text-sm" : "w-10 h-10 text-base";
