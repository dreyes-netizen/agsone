# Dashboard Feed Reactions & Comments — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add inline reactions (hover-to-reveal emoji picker) and threaded comments (latest 2, full thread on expand) to each post card in the Home dashboard feed widget.

**Architecture:** Create a self-contained `DashboardFeedCard` client component that owns all reaction + comment state internally. The dashboard page just maps over posts and renders one `<DashboardFeedCard />` per post. The feed page (`/feed`) is not touched.

**Tech Stack:** Next.js 16 (App Router), React 19, TypeScript, Tailwind CSS, Lucide icons, `useApiClient` hook for authenticated fetches.

**Spec:** `docs/superpowers/specs/2026-06-04-dashboard-feed-reactions-comments-design.md`
...
|---|---|---|
| **Create** | `components/dashboard/DashboardFeedCard.tsx` | Self-contained card: static layout, reactions, comments, replies |
| **Modify** | `app/(dashboard)/dashboard/page.tsx` | Add `myReactions` to `FeedPost` type; swap inline card render with `<DashboardFeedCard />` |

---

## Task 1: Create `DashboardFeedCard.tsx`

**Files:**
- Create: `components/dashboard/DashboardFeedCard.tsx`

### Step 1.1 — Create the file with types, constants, and sub-components

Create `components/dashboard/DashboardFeedCard.tsx` with exactly this content:
...
Do **not** close the file yet — continue in step 1.2.

- [ ] Create the file with content above

### Step 1.2 — Add the main `DashboardFeedCard` export

Append the following to the same file (after the `ReactionBar` function):
            ...
            (initialPost.imageUrls.length === 1 ? (
              ...
              <div className="mt-2 grid grid-cols-2 gap-1 rounded-lg overflow-hidden">
                {initialPost.imageUrls.slice(0, 4).map((url, idx) => (
                  <div key={idx} className="relative aspect-square bg-zinc-100 overflow-hidden">
...
- [ ] Append the `DashboardFeedCard` export to the file
...
Expected: no errors related to `components/dashboard/DashboardFeedCard.tsx`. If there are errors, fix them before continuing.
...
### Step 2.1 — Add `myReactions` to the `FeedPost` type and import `DashboardFeedCard`
...
**b) Update the `FeedPost` type** (around line 23) — add `myReactions`:
...
- [ ] Add import and update type

### Step 2.2 — Replace the inline feed card render with `<DashboardFeedCard />`

Find the `feedPosts.map(...)` block inside the "What's happening" widget (around line 249). It looks like this:
...
feedPosts.map((post) => (
  <div
    key={post.id}
    onClick={() => router.push("/feed")}
    className={`bg-white rounded-xl border p-4 hover:shadow-sm transition-shadow cursor-pointer ${post.type === "SHOUTOUT" ? "border-l-4 border-l-pink-400 border-zinc-100" : "border-zinc-100"}`}
  >
    ...entire card body including avatar, content, images, reactions display...
  </div>
))
```

Replace the entire `feedPosts.map(...)` expression (from `feedPosts.map(` down to and including the closing `))`) with:
...
- [ ] Replace inline card render with `<DashboardFeedCard />`

### Step 2.3 — Remove unused imports from `dashboard/page.tsx`

After swapping in `DashboardFeedCard`, the following imports are no longer used directly in `dashboard/page.tsx` and should be removed if they are not used elsewhere in the file:

- `Rss` from `lucide-react` (still used in the empty-state placeholder — **keep it**)
- `postTimestamp` from `@/lib/helpers/timeAgo` — check if still used; if the only use was inside the feed card render, remove it
- The local `renderContent` function (lines ~82–104) — used only in the feed card render; delete it if no longer referenced
- The local `Avatar` function (lines ~106–116) — used in leaderboard, shoutouts, and birthdays widgets; **keep it**

Run TypeScript to surface any "declared but never used" errors and clean them up.

```bash
npx tsc --noEmit
```

- [ ] Remove unused imports/functions and fix TypeScript errors

### Step 2.4 — Commit

```bash
git add components/dashboard/DashboardFeedCard.tsx app/(dashboard)/dashboard/page.tsx
git commit -m "feat: add inline reactions and comments to dashboard feed widget"
```
...
Start the dev server and open the dashboard in a browser.

```bash
npm run dev
```

Open `http://localhost:3000/dashboard` (or whatever port the dev server uses).

**Test checklist:**

- [ ] Feed posts render correctly with no visual regressions (author, content, images, shoutout styling)
- [ ] Hovering the "React" button after 350ms shows the floating emoji picker
- [ ] Clicking an emoji adds it as a reaction bubble; count increments; button shows the emoji + label
- [ ] Clicking the active emoji button again removes the reaction (toggle off)
- [ ] Clicking the "X comments" button opens the comment section inline
- [ ] If a post has > 2 comments, "View X earlier comments" link appears; clicking it reveals all
- [ ] Typing in the comment box and pressing Enter (or clicking send) posts a comment optimistically
- [ ] The new comment appears immediately; comment count increments
- [ ] "Reply" under a comment shows the reply input; Enter submits; the reply appears under its parent
- [ ] "View N replies" toggles the reply thread open/closed
- [ ] Clicking the post body (not a button) still navigates to `/feed`
- [ ] No click-through to `/feed` when interacting with buttons, inputs, or links inside the card

---

## Self-Review Notes

- **Spec coverage:** All requirements covered — inline reactions (hover picker), comments (latest 2 + view earlier), full thread on expand, threading (replies), stopPropagation on all interactive elements, error states for fetch failure and comment/reply submit failure.
- **No placeholders:** All code blocks are complete.
- **Type consistency:** `DashboardFeedPost` type (exported from `DashboardFeedCard.tsx`) matches the updated `FeedPost` type in `dashboard/page.tsx` — both include `myReactions: string[]`. The `CommentItem` and `ReplyItem` types in `DashboardFeedCard.tsx` match the shape returned by `GET /api/feed/{id}/comments`.
